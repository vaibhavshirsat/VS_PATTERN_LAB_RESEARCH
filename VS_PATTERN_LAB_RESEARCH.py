...
with st.sidebar:
    ...
    universe=st.radio("Universe",["Top 100 Stocks","Stocks","Indices","Crypto","Commodities","Global Assets","Custom"],key="side_universe")
    if universe=="Top 100 Stocks":
        market="India"
        symbols=INDIA_TOP_100
        st.caption("100 NSE research stocks selected")
    elif universe=="Stocks":
        base=INDIA_TOP_100 if market=="India" else GLOBAL_STOCKS
        symbols=st.multiselect("Symbols",base,default=base[:10],key="side_symbols")
    elif universe=="Indices":
        available=list(INDIA_INDEXES) if market=="India" else list(GLOBAL_INDEXES)
        names=st.multiselect("Indices",available,default=available[:5],key="side_indices"); symbols=[INDEX_SYMBOLS[x] for x in names]
    elif universe=="Crypto":
        names=st.multiselect("Crypto",list(CRYPTO_SYMBOLS),default=list(CRYPTO_SYMBOLS)[:8],key="side_crypto"); symbols=[CRYPTO_SYMBOLS[x] for x in names]; market="Global"
    elif universe=="Commodities":
        names=st.multiselect("Commodities",list(COMMODITIES),default=list(COMMODITIES)[:4],key="side_commodities"); symbols=[COMMODITIES[x] for x in names]; market="Global"
    elif universe=="Global Assets":
        names=st.multiselect("Global stocks / indices",GLOBAL_STOCKS+list(GLOBAL_INDEXES),default=GLOBAL_STOCKS[:10],key="side_global_assets"); symbols=names; market="Global"
    else:
