...
MAJOR_INDIAN_INDEXES = {k: INDEX_SYMBOLS[k] for k in ["NIFTY 50","BANK NIFTY","FINNIFTY","NIFTY MIDCAP SELECT","NIFTY IT","NIFTY PHARMA","NIFTY FMCG"]}
MAJOR_CRYPTO = {k: CRYPTO_SYMBOLS[k] for k in ["Bitcoin","Ethereum","BNB","Solana","XRP","Cardano","Dogecoin"]}
RESEARCH_SYMBOL_GROUPS = {
    "ALL • INDIA TOP 100 STOCKS": INDIA_TOP_100,
    "ALL • INDIA MAJOR 7 INDICES": list(MAJOR_INDIAN_INDEXES.values()),
    "ALL • GLOBAL STOCKS": GLOBAL_STOCKS,
    "ALL • GLOBAL INDICES": list(GLOBAL_INDEXES.values()),
    "ALL • GLOBAL ASSETS": list(GLOBAL_INDEXES.values()) + list(COMMODITIES.values()) + list(FX_ASSETS.values()) + list(RATES.values()),
    "ALL • MAJOR COMMODITIES": list(COMMODITIES.values()),
    "ALL • MAJOR FOREX": list(FX_ASSETS.values()),
    "ALL • MAJOR CRYPTO": list(MAJOR_CRYPTO.values()),
}
...
def scan_universe_mtf(symbols, market):
    """Seven-timeframe pattern scan; 4H is the primary research timeframe."""
    symbols=list(dict.fromkeys([str(x).strip() for x in symbols if str(x).strip()]))
    jobs=[(sym,tf) for sym in symbols for tf in TFS]
    rows=[]
    workers=min(12,max(4,len(jobs)))
    with ThreadPoolExecutor(max_workers=workers) as pool:
        futures={pool.submit(scan_symbol,sym,tf,market):(sym,tf) for sym,tf in jobs}
        for fut in as_completed(futures):
            sym,tf=futures[fut]
            try: rows.extend(fut.result())
            except Exception as e:
                rows.append({"Symbol":sym,"Pattern":"ERROR","Category":"System","TF":tf,"Direction":"Neutral","Status":"Error","Score":0,"LifecycleScore":0,"Context":str(e)})
    return pd.DataFrame(rows)


# ===== mtf.py =====

TFS=["1MN","1W","1D","4H","1H","15M","5M"]
...
# ===== ADD-ON — ELLIOTT WAVE + FIBONACCI RESEARCH =====
def fibonacci_elliott_research(df):
    """Heuristic swing-based Elliott/Fibonacci research; not an automated trade signal."""
    if df is None or df.empty or len(df) < 30:
        return {"status":"Insufficient data"}, pd.DataFrame()
    x=df.dropna(subset=["High","Low","Close"]).copy()
    highs,lows=pivot_points(x,left=3,right=3)
    piv=sorted([(i,float(v),"High") for i,v in highs]+[(i,float(v),"Low") for i,v in lows],key=lambda z:z[0])
    clean=[]
    for p in piv:
        if clean and clean[-1][2]==p[2]:
            if (p[2]=="High" and p[1]>clean[-1][1]) or (p[2]=="Low" and p[1]<clean[-1][1]): clean[-1]=p
        else: clean.append(p)
    if len(clean)<2: return {"status":"Not enough confirmed swing pivots"},pd.DataFrame()
    a,b=clean[-2],clean[-1]
    start,end=sorted([a[1],b[1]]); span=end-start
    if span<=0: return {"status":"Invalid swing range"},pd.DataFrame()
    direction="Bullish" if b[2]=="High" else "Bearish"
    if direction=="Bullish":
        fibs=[("23.6%",end-span*.236),("38.2%",end-span*.382),("50.0%",end-span*.5),("61.8%",end-span*.618),("78.6%",end-span*.786)]
        targets=[("1.000x",end+span),("1.272x",end+span*1.272),("1.618x",end+span*1.618),("2.000x",end+span*2)]
    else:
        fibs=[("23.6%",start+span*.236),("38.2%",start+span*.382),("50.0%",start+span*.5),("61.8%",start+span*.618),("78.6%",start+span*.786)]
        targets=[("1.000x",start-span),("1.272x",start-span*1.272),("1.618x",start-span*1.618),("2.000x",start-span*2)]
    rows=[{"Layer":"Retracement","Level":k,"Price":round(v,2)} for k,v in fibs]
    rows += [{"Layer":"Extension (illustrative)","Level":k,"Price":round(v,2)} for k,v in targets]
    rows += [{"Layer":"Recent swing pivot","Level":p[2],"Price":round(p[1],2),"Date":str(x.index[p[0]])} for p in clean[-5:]]
    latest_close=float(x.Close.iloc[-1])
    # Treat the latest price crossing the origin of the mapped swing as a structural invalidation.
    if direction=="Bullish":
        swing_validity="Invalidated — latest close below swing low" if latest_close < start else "Not invalidated by swing-low break"
    else:
        swing_validity="Invalidated — latest close above swing high" if latest_close > end else "Not invalidated by swing-high break"
    metrics={"Status":"Heuristic swing map","Swing direction":direction,"Swing validity":swing_validity,
             "Swing start":round(a[1],2),"Swing end":round(b[1],2),
             "Latest close":round(latest_close,2),"Swing size":round(span,2),
             "Fib context":"Historical swing levels only — not a live entry signal",
             "Wave count":"Not confirmed — swing pivots only",
             "Caution":"Elliott counts are subjective; verify manually across timeframes."}
    return metrics,pd.DataFrame(rows)

def _make_research_pdf(title, metrics, table):
    import io
    ...
    for k,v in metrics.items(): story.append(Paragraph(f"<b>{k}:</b> {v}",styles["BodyText"]))
    story.append(Spacer(1,12))
    data=[list(table.columns)]+table.fillna("").astype(str).values.tolist() if not table.empty else [["No data"]]
    tbl=Table(data,repeatRows=1)
    tbl.setStyle(TableStyle([("BACKGROUND",(0,0),(-1,0),colors.HexColor("#12334a")),("TEXTCOLOR",(0,0),(-1,0),colors.white),("GRID",(0,0),(-1,-1),.35,colors.grey),("FONTSIZE",(0,0),(-1,-1),8),("VALIGN",(0,0),(-1,-1),"TOP"),("ROWBACKGROUNDS",(0,1),(-1,-1),[colors.whitesmoke,colors.lightgrey])]))
...
with st.sidebar:
    ...
    nav=[("🏠","HOME"),("🔎","PATTERN SCANNER"),("🕯️","CANDLE SCREENER"),("🧭","MTF RESEARCH"),("📚","HISTORICAL RESEARCH"),("📈","CHART LAB"),("🧩","PATTERN UNIVERSE"),("🌍","MARKET UNIVERSE"),("〰️","ELLIOTT + FIBONACCI")]
    ...
    scan_mode=st.selectbox("Pattern Scan Mode",["SINGLE TIMEFRAME","MTF • ALL 7 TIMEFRAMES"],index=0,key="scan_mode_final")
...
if run:
    with st.spinner("Running pattern + context engines..."):
        st.session_state.scan=scan_universe_mtf(symbols,market) if scan_mode=="MTF • ALL 7 TIMEFRAMES" else scan_universe(symbols,tf,market)
...
if st.session_state.page=="HOME":
    ...
    if scan_here:
        scan_scope = "7 TF" if scan_mode=="MTF • ALL 7 TIMEFRAMES" else tf
        with st.spinner(f"Scanning {len(symbols)} symbols on {scan_scope}…"):
            st.session_state.scan=scan_universe_mtf(symbols,market) if scan_mode=="MTF • ALL 7 TIMEFRAMES" else scan_universe(symbols,tf,market)
        res=st.session_state.scan
    scan_label="7-TF MTF" if scan_mode=="MTF • ALL 7 TIMEFRAMES" else f"TIMEFRAME {tf}"
    st.markdown(f'<div class="panel"><span class="pill green">ACTIVE UNIVERSE {len(symbols)}</span><span class="pill">{scan_label}</span><span class="pill gold">PATTERN + CANDLE + GEOMETRY</span><span class="pill">4H PRIMARY</span></div>',unsafe_allow_html=True)
    ...
    research_group_options=list(RESEARCH_SYMBOL_GROUPS.keys())
    rg=st.selectbox("Research Universe",research_group_options,index=0,key="research_group_final")
    group_symbols=RESEARCH_SYMBOL_GROUPS[rg]
    if rg=="ALL • INDIA TOP 100 STOCKS": label_to_symbol={x:x for x in INDIA_TOP_100}
    elif rg=="ALL • INDIA MAJOR 7 INDICES": label_to_symbol={f"{k} • {v}":v for k,v in MAJOR_INDIAN_INDEXES.items()}
    elif rg=="ALL • GLOBAL STOCKS": label_to_symbol={x:x for x in GLOBAL_STOCKS}
    elif rg=="ALL • GLOBAL INDICES": label_to_symbol={f"{k} • {v}":v for k,v in GLOBAL_INDEXES.items()}
    elif rg=="ALL • GLOBAL ASSETS": label_to_symbol={f"{k} • {v}":v for k,v in {**GLOBAL_INDEXES,**COMMODITIES,**FX_ASSETS,**RATES}.items()}
    elif rg=="ALL • MAJOR COMMODITIES": label_to_symbol={f"{k} • {v}":v for k,v in COMMODITIES.items()}
    elif rg=="ALL • MAJOR FOREX": label_to_symbol={f"{k} • {v}":v for k,v in FX_ASSETS.items()}
    else: label_to_symbol={f"{k} • {v}":v for k,v in MAJOR_CRYPTO.items()}
    symbol_choices=["ALL • RUN THIS RESEARCH UNIVERSE"]+list(label_to_symbol.keys())
    preferred="RELIANCE" if "INDIA" in rg else "AAPL"
    default_idx=next((i for i,x in enumerate(symbol_choices) if label_to_symbol.get(x)==preferred),1 if len(symbol_choices)>1 else 0)
    rs=st.selectbox("Research Symbol",symbol_choices,index=default_idx,key="mtf_symbol_final")
    selected_market="India" if "INDIA" in rg else "Global"
    st.caption(f"Research set: {len(group_symbols)} instruments • 7 TF: 1MN → 1W → 1D → 4H → 1H → 15M → 5M • 4H PRIMARY")
    ...
    if st.button("RUN MTF RESEARCH",type="primary",key="run_mtf_v3"):
        selected_symbols=group_symbols if rs.startswith("ALL •") else [label_to_symbol[rs]]
        if len(selected_symbols)>1:
            st.info(f"Running 7-timeframe MTF research across {len(selected_symbols)} instruments. 4H is primary.")
            mtf_rows=[]
            with st.spinner("Running 7-TF pattern + trendline research…"):
                for ss in selected_symbols:
                    mtmp,_=mtf_analysis(ss,selected_market); mtmp.insert(0,"Symbol",ss); mtf_rows.append(mtmp)
            mtf=pd.concat(mtf_rows,ignore_index=True) if mtf_rows else pd.DataFrame()
            summary={"FinalBias":"Multi-symbol research","BullWeighted":0,"BearWeighted":0,"TrendlineBias":"See table"}
        else:
            mtf,summary=mtf_analysis(selected_symbols[0],selected_market)
        a,b,c,d=st.columns(4);a.metric("Final Pattern Bias",summary["FinalBias"]);b.metric("Bull Weighted",summary["BullWeighted"]);c.metric("Bear Weighted",summary["BearWeighted"]);d.metric("Trendline Bias",summary["TrendlineBias"]);
        st.markdown('### 📐 MTF TRENDLINE MAP — ALL 7 TIMEFRAMES')
        st.markdown('<div class="panel"><b>Trendline is an independent structure layer.</b> Every timeframe gets its own support/resistance trendline state, interaction, position and strength. <span class="gold"><b>4H is PRIMARY.</b></span></div>',unsafe_allow_html=True)
        tl_cols=["TF","TrendlineBias","SupportTrendline","ResistanceTrendline","TrendlineEvent","TrendlinePosition","TLStrength"]
        st.dataframe(mtf[tl_cols],use_container_width=True,hide_index=True,height=390)
        st.markdown('### ⭐ 4H PRIMARY TRENDLINE')
        tl4=mtf[mtf.TF=="4H"]
        if not tl4.empty:
            r=tl4.iloc[0]
            st.markdown(f'<div class="panel"><span class="pill gold">4H PRIMARY</span><span class="pill">{r.TrendlineBias}</span><span class="pill">Support: {r.SupportTrendline}</span><span class="pill">Resistance: {r.ResistanceTrendline}</span><br><br><b>Interaction:</b> {r.TrendlineEvent}<br><b>Position:</b> {r.TrendlinePosition}<br><b>Strength:</b> {r.TLStrength}/100</div>',unsafe_allow_html=True)
...
elif st.session_state.page=="ELLIOTT + FIBONACCI":
    st.markdown('<div class="section">〰️ ELLIOTT WAVE + FIBONACCI <small>heuristic swing map • retracement • extension • CSV/PDF</small></div>',unsafe_allow_html=True)
    st.markdown('<div class="panel"><b>Workflow:</b> Manually review 1MN → 1W → 1D → 4H → 1H → 15M → 5M. This module does not confirm an Elliott count or generate entries.</div>',unsafe_allow_html=True)
    universe_choice=st.selectbox("Research universe",["India Top 100 Stocks","India Major 7 Indices","Top Global Stocks","Global Indices","Major Crypto"],key="ewf_universe")
    universe_map={
        "India Top 100 Stocks":(INDIA_TOP_100,"India"),
        "India Major 7 Indices":(list(MAJOR_INDIAN_INDEXES.values()),"Global"),
        "Top Global Stocks":(GLOBAL_STOCKS,"Global"),
        "Global Indices":(list(GLOBAL_INDEXES.values()),"Global"),
        "Major Crypto":(list(MAJOR_CRYPTO.values()),"Global")}
    universe_symbols,ewf_market=universe_map[universe_choice]
    chosen_symbol=st.selectbox("Symbol",universe_symbols,key="ewf_symbol")
    ewf_tf=st.selectbox("Timeframe",TFS,index=3,key="ewf_tf")
    if st.button("RUN ELLIOTT + FIBONACCI RESEARCH",type="primary",key="run_ewf"):
        with st.spinner(f"Fetching {chosen_symbol} • {ewf_tf}…"):
            ewf_df=fetch_ohlcv(chosen_symbol,ewf_tf,ewf_market)
        if ewf_df.empty: st.error("No market data returned for this symbol/timeframe.")
        else:
            ewf_metrics,ewf_table=fibonacci_elliott_research(ewf_df)
            st.session_state["ewf_metrics"]=ewf_metrics; st.session_state["ewf_table"]=ewf_table
            st.session_state["ewf_symbol_result"]=chosen_symbol; st.session_state["ewf_tf_result"]=ewf_tf
    if "ewf_metrics" in st.session_state:
        metrics=st.session_state["ewf_metrics"]; table=st.session_state["ewf_table"]
        st.markdown("### Swing context")
        for key,value in metrics.items(): st.write(f"**{key}:** {value}")
        st.markdown("### Fibonacci levels + recent swing pivots")
        st.dataframe(table,use_container_width=True,hide_index=True)
        export_df=table.copy()
        export_df.insert(0,"Symbol",st.session_state.get("ewf_symbol_result",""))
        export_df.insert(1,"Timeframe",st.session_state.get("ewf_tf_result",""))
        st.download_button("⬇ DOWNLOAD CSV",export_df.to_csv(index=False).encode("utf-8"),"vs_flow_elliott_fibonacci.csv","text/csv",key="ewf_csv")
        try:
            pdf_bytes=_make_research_pdf(f"VS FLOW — Elliott + Fibonacci | {st.session_state.get('ewf_symbol_result','')} • {st.session_state.get('ewf_tf_result','')}",metrics,table)
            st.download_button("⬇ DOWNLOAD PDF",pdf_bytes,"vs_flow_elliott_fibonacci.pdf","application/pdf",key="ewf_pdf")
        except Exception as e: st.info(f"PDF export unavailable in this environment: {e}")
        st.caption("Research aid only. Validate swing selection, Elliott count, levels and risk manually; no trade is generated.")
