...
INDEX_SYMBOLS = {
    # 7 major Indian research indices
    "NIFTY 50":"^NSEI", "BANK NIFTY":"^NSEBANK", "FINNIFTY":"^CNXFIN",
    "MIDCAP SELECT":"^NSEMDCP50", "NIFTY IT":"^CNXIT", "NIFTY PHARMA":"^CNXPHARMA",
    "NIFTY FMCG":"^CNXFMCG",
    # Global benchmarks
    "S&P 500":"^GSPC", "NASDAQ":"^IXIC", "DOW JONES":"^DJI",
    "NIKKEI":"^N225", "HANG SENG":"^HSI"
}
INDIA_INDEXES = {k:v for k,v in INDEX_SYMBOLS.items() if k in ["NIFTY 50","BANK NIFTY","FINNIFTY","MIDCAP SELECT","NIFTY IT","NIFTY PHARMA","NIFTY FMCG"]}
CRYPTO_SYMBOLS = {"Bitcoin":"BTC-USD","Ethereum":"ETH-USD","BNB":"BNB-USD","Solana":"SOL-USD","XRP":"XRP-USD","Dogecoin":"DOGE-USD","Cardano":"ADA-USD"}
...
with st.sidebar:
    ...
    tf=st.selectbox("Timeframe",TFS,index=4,key="side_tf")
    universe=st.radio("Universe",["Stocks","Indices","Crypto","Custom"],key="side_universe")
    if universe=="Stocks":
        base=INDIA_DEFAULT if market=="India" else GLOBAL_DEFAULT; symbols=st.multiselect("Symbols",base,default=base[:10],key="side_symbols")
    elif universe=="Indices":
        names=st.multiselect("Indices",list(INDEX_SYMBOLS),default=list(INDIA_INDEXES)[:3] if market=="India" else list(INDEX_SYMBOLS)[:3],key="side_indices"); symbols=[INDEX_SYMBOLS[x] for x in names]
    elif universe=="Crypto":
        names=st.multiselect("Crypto",list(CRYPTO_SYMBOLS),default=list(CRYPTO_SYMBOLS)[:5],key="side_crypto"); symbols=[CRYPTO_SYMBOLS[x] for x in names]; market="Global"
    else:
...
if st.session_state.page=="HOME":
    ...
    st.markdown('<div class="section">⚡ ONE-CLICK MARKET RESEARCH <small>7 Indian indices • 7 crypto • instant context</small></div>',unsafe_allow_html=True)
    qitems=[("NIFTY 50","^NSEI"),("BANK NIFTY","^NSEBANK"),("FINNIFTY","^CNXFIN"),("MIDCAP SELECT","^NSEMDCP50"),("NIFTY NEXT 50","^NSMIDCP"),("NIFTY IT","^CNXIT"),("NIFTY FMCG","^CNXFMCG")]
    cols=st.columns(7)
    for col,(label,symq) in zip(cols,qitems):
        with col:
    ...
    st.markdown('<div class="panel"><b>Professional workflow:</b> Market Context → Pattern Detection → Lifecycle → MTF Alignment → Historical Validation → Visual Confirmation.</div>',unsafe_allow_html=True)
    ...
    if res.empty: st.info("Run the research scan from the left panel.")
        ...
        st.dataframe(view,use_container_width=True,hide_index=True,height=500); st.download_button("⬇ DOWNLOAD SCAN CSV",view.to_csv(index=False).encode("utf-8"),"vs_pattern_lab_scan.csv","text/csv",key="dl_scan_v3",on_click="ignore")
    ...
    cs_symbols=symbols if symbols else (list(INDIA_INDEXES.values()) if market=="India" else list(GLOBAL_DEFAULT))
    if st.button("RUN CANDLE SCREENER",type="primary",key="run_candle_v3"):
        ...
        for ss in cs_symbols:
            try:
                ...
                last=detect_candles(cdf).iloc[-1]
                for pat in [p for p in detect_candles(cdf).columns if bool(last[p])]:
    ...
    st.markdown('<div class="section">🧭 MULTI-TIMEFRAME RESEARCH <small>5M → 1MN weighted pattern intelligence</small></div>',unsafe_allow_html=True)
    ...
    if st.button("RUN MTF RESEARCH",type="primary",key="run_mtf_v3"):
        mtf,summary=mtf_analysis(sym,market);a,b,c,d=st.columns(4);a.metric("Final Bias",summary["FinalBias"]);b.metric("Bull Weighted",summary["BullWeighted"]);c.metric("Bear Weighted",summary["BearWeighted"]);d.metric("Timeframes",len(mtf));st.dataframe(mtf,use_container_width=True,hide_index=True);st.download_button("⬇ DOWNLOAD MTF CSV",mtf.to_csv(index=False).encode("utf-8"),"vs_pattern_lab_mtf.csv","text/csv",key="dl_mtf_v3",on_click="ignore")
    ...
    bsym=st.text_input("Backtest symbol",symbols[0] if symbols else ("RELIANCE" if market=="India" else "AAPL"),key="bt_symbol_v3"); btf=st.selectbox("Backtest timeframe",TFS,index=4,key="btf_v3"); direction=st.selectbox("Research direction",["Bullish","Bearish"],key="btdir_v3"); horizon=st.slider("Forward candles",1,50,10,key="horizon_v3")
    ...
    rows=[]
    for name,sym in INDEX_SYMBOLS.items(): rows.append({"Type":"INDEX","Name":name,"Symbol":sym})
    for name,sym in CRYPTO_SYMBOLS.items(): rows.append({"Type":"CRYPTO","Name":name,"Symbol":sym})
    for sym in list(INDIA_DEFAULT)+list(GLOBAL_DEFAULT): rows.append({"Type":"STOCK","Name":sym,"Symbol":sym})
    st.dataframe(pd.DataFrame(rows).drop_duplicates("Symbol"),use_container_width=True,hide_index=True,height=560)
